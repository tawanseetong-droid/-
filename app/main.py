...
import os, sqlite3, asyncio, json, re, io, zipfile
...
<section class=stats><div class=stat><div class=label>รายการทั้งหมด</div><div class=num id=total>0</div></div><div class=stat><div class=label>แจกฟรี</div><div class=num id=free>0</div></div><div class=stat><div class=label>คูปอง/ส่วนลด</div><div class=num id=coupon>0</div></div><div class=stat><div class=label>ลิงก์ตรวจแล้ว</div><div class=num id=live>0</div></div></section><section class=panel><div class=panel-title><div><h2>ใช้คูปองจากบัญชีของคุณ</h2><div class=desc>ติดตั้ง Money Hunter Companion แล้วเปิดหน้าคูปองในบัญชี จากนั้นกดไอคอนส่วนขยายเพื่อค้นหาและเรียงรายการบนเครื่องของคุณ</div></div><a class=claim href="/companion.zip">ดาวน์โหลด Companion</a></div></section>
...
COMPANION_MANIFEST='''{"manifest_version":3,"name":"Money Hunter Companion","version":"1.0.0","description":"ค้นหาและเรียงคูปองที่มองเห็นในบัญชีของคุณ โดยไม่ส่งข้อมูลออกจากเครื่อง","permissions":["activeTab","scripting"],"action":{"default_title":"Money Hunter","default_popup":"popup.html"},"content_security_policy":{"extension_pages":"script-src 'self'; object-src 'self'"}}'''
COMPANION_HTML='''<!doctype html><html lang="th"><head><meta charset="utf-8"><style>*{box-sizing:border-box}body{width:390px;min-height:260px;margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f4f7fb;color:#142238}.head{background:linear-gradient(135deg,#071426,#0e7490);color:#fff;padding:17px}.head b{font-size:18px}.head div{font-size:12px;color:#d8f1f5;margin-top:5px}.bar{padding:12px}.scan{width:100%;border:0;border-radius:12px;padding:12px;background:#fbbf24;font-weight:850;cursor:pointer}.note{font-size:11px;color:#6b778b;margin:9px 1px}.list{display:grid;gap:8px}.item{background:#fff;border:1px solid #e2e8f0;border-radius:13px;padding:11px}.tags{display:flex;gap:5px;margin-bottom:5px}.tag{font-size:10px;border-radius:99px;padding:4px 7px;background:#eef2f7}.value{background:#dcfce7;color:#166534;font-weight:800}.title{font-size:13px;font-weight:750;line-height:1.35}.go{display:block;text-align:center;text-decoration:none;background:#1565d8;color:#fff;border-radius:9px;padding:8px;margin-top:8px;font-size:12px;font-weight:800}.empty{text-align:center;color:#718096;padding:28px 8px}</style></head><body><div class=head><b>฿ Money Hunter Companion</b><div>อ่านเฉพาะคูปองที่แสดงอยู่ในแท็บปัจจุบัน</div></div><div class=bar><button id=scan class=scan>ค้นหาและเรียงคูปองหน้านี้</button><div class=note>ข้อมูลประมวลผลในเครื่อง ไม่ส่งรหัสผ่าน คุกกี้ หรือ OTP ออกไป</div><div id=list class=list></div></div><script src="popup.js"></script></body></html>'''
COMPANION_JS=r'''const $=s=>document.querySelector(s);const esc=s=>String(s||'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));
function scanPage(){const host=location.hostname.toLowerCase();const platform=host.includes('shopee')?'Shopee':host.includes('lazada')?'Lazada':host.includes('tiktok')?'TikTok Shop':'';if(!platform)return {error:'กรุณาเปิด TikTok Shop, Shopee หรือ Lazada ก่อน'};const keys=['คูปอง','โค้ด','ส่วนลด','ส่งฟรี','แจกฟรี','รับฟรี','voucher','coupon','discount','cashback','coin'];const seen=new Set(),out=[];for(const el of document.querySelectorAll('a,button,[role="button"],article,li')){const text=(el.innerText||el.textContent||'').replace(/\s+/g,' ').trim();if(text.length<4||text.length>260||!keys.some(k=>text.toLowerCase().includes(k)))continue;let a=el.matches('a')?el:el.closest('a')||el.querySelector('a');let url=a&&a.href?a.href:location.href;if(!/^https?:/.test(url))continue;let key=text+'|'+url;if(seen.has(key))continue;seen.add(key);let pc=[...text.matchAll(/(\d+(?:\.\d+)?)\s*%/g)].map(x=>+x[1]);let ba=[...text.replace(/,/g,'').matchAll(/(?:ลด|คืน|รับ)\s*(?:สูงสุด\s*)?(\d+(?:\.\d+)?)\s*(?:บาท|฿)/g)].map(x=>+x[1]);let free=/(แจกฟรี|รับฟรี|ฟรีทั้งหมด|100%\s*free)/i.test(text),amount=Math.max(0,...ba),percent=Math.max(0,...pc),score=free?1e9:amount?1e8+amount:percent?1e7+percent:text.includes('ส่งฟรี')?1e6:0;out.push({platform,text,url,free,amount,percent,score});if(out.length>=100)break}out.sort((a,b)=>b.score-a.score);return {platform,items:out}}
function badge(x){if(x.free)return 'ฟรี';if(x.amount)return 'ลด ฿'+x.amount.toLocaleString();if(x.percent)return 'ลด '+x.percent+'%';if(x.text.includes('ส่งฟรี'))return 'ส่งฟรี';return 'คูปอง'}function render(r){if(r.error){$('#list').innerHTML='<div class=empty>'+esc(r.error)+'</div>';return}if(!r.items.length){$('#list').innerHTML='<div class=empty>ยังไม่พบคูปองในหน้าที่เปิดอยู่<br>ลองเข้าหน้าคูปองหรือเลื่อนให้รายการแสดงก่อน</div>';return}$('#list').innerHTML=r.items.map(x=>`<div class=item><div class=tags><span class=tag>${esc(x.platform)}</span><span class="tag value">${esc(badge(x))}</span></div><div class=title>${esc(x.text)}</div><a class=go href="${esc(x.url)}" target=_blank>เปิดรายการนี้ →</a></div>`).join('')}
$('#scan').addEventListener('click',async()=>{let [tab]=await chrome.tabs.query({active:true,currentWindow:true});if(!tab||!tab.id)return render({error:'ไม่พบแท็บที่เปิดอยู่'});$('#scan').textContent='กำลังค้นหา…';try{let [{result}]=await chrome.scripting.executeScript({target:{tabId:tab.id},func:scanPage});render(result)}catch(e){render({error:'หน้านี้ไม่อนุญาตให้อ่าน กรุณารีเฟรชแล้วลองใหม่'})}finally{$('#scan').textContent='ค้นหาและเรียงคูปองหน้านี้'}});'''

@app.get('/companion.zip')
def companion_zip():
 buf=io.BytesIO()
 with zipfile.ZipFile(buf,'w',zipfile.ZIP_DEFLATED) as z:
  z.writestr('money-hunter-companion/manifest.json',COMPANION_MANIFEST);z.writestr('money-hunter-companion/popup.html',COMPANION_HTML);z.writestr('money-hunter-companion/popup.js',COMPANION_JS)
 return Response(buf.getvalue(),media_type='application/zip',headers={'Content-Disposition':'attachment; filename=money-hunter-companion.zip'})
