...
app=FastAPI(title='Money Hunter AI Cloud',version='10.2')
...
async def scan_once():
 added=0
 async with httpx.AsyncClient(headers={'User-Agent':'MoneyHunterAI/10.2'},follow_redirects=True,timeout=20) as client:
...
def readiness():
 ...
 return {'discovery':True,'live_money_actions':False,'live_payouts':False,'provider_connected':configured,'paypal_mode':os.getenv('PAYPAL_MODE','sandbox'),'message':'PayPal credentials พร้อมทดสอบ' if configured else 'ค้นหาได้แล้ว; รอเชื่อม provider'}
@app.get('/health')
def health():return {'ok':True,'service':'money-hunter-ai','version':'10.2'}

HTML='''<!doctype html><html lang="th"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Money Hunter AI</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f6f7fb;margin:0;color:#18202a}.wrap{max-width:760px;margin:auto;padding:20px}.hero{background:#111827;color:#fff;border-radius:24px;padding:24px}.money{font-size:42px;font-weight:800;margin:8px 0}.btn{border:0;border-radius:14px;padding:14px 18px;font-size:16px;font-weight:700;cursor:pointer}.primary{background:#22c55e;color:#06240f;width:100%;margin-top:14px}.secondary{background:#e5e7eb;color:#111827;width:100%;margin-top:10px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:14px}.card{background:#fff;border-radius:18px;padding:16px;box-shadow:0 4px 18px #0000000d}.deal{margin-top:10px;border-top:1px solid #eee;padding-top:12px}.tag{display:inline-block;background:#eef2ff;padding:5px 9px;border-radius:999px;font-size:12px}.muted{color:#6b7280;font-size:13px}a{color:#2563eb;text-decoration:none}</style></head><body><div class=wrap><div class=hero><div>Money Hunter AI</div><div class=muted style="color:#cbd5e1">ค้นหา → ตรวจสิทธิ์ → ยืนยันจ่าย → ถอน THB</div><div class=money id=money>฿0.00</div><div>เงินจริงที่ยืนยันแล้ว</div><button id=scanBtn class="btn primary" onclick="scan()">ค้นหาให้ตอนนี้</button><button id=paypalBtn class="btn secondary" onclick="testPaypal()">ทดสอบ PayPal Sandbox</button></div><div class=grid><div class=card><b>สถานะระบบ</b><div id=ready class=muted>กำลังตรวจ...</div><div id=paypal class=muted style="margin-top:6px"></div></div><div class=card><b>รายการที่พบ</b><div id=count style="font-size:30px;font-weight:800">0</div></div></div><div class=card style="margin-top:14px"><b>โอกาสล่าสุด</b><div id=deals></div></div></div><script>async function load(){let w=await fetch('/api/wallet').then(r=>r.json());money.textContent='฿'+Number(w.confirmed_thb||0).toLocaleString('th-TH',{minimumFractionDigits:2,maximumFractionDigits:2});let rs=await fetch('/api/readiness').then(r=>r.json());ready.textContent=rs.message;let ps=await fetch('/api/paypal/status').then(r=>r.json());paypal.textContent=ps.configured?'PayPal: พร้อมทดสอบ ('+ps.mode+')':'PayPal: ยังตั้งค่าไม่ครบ';let ds=await fetch('/api/deals').then(r=>r.json());count.textContent=ds.length;deals.innerHTML=ds.slice(0,20).map(d=>`<div class=deal><span class=tag>${d.kind}</span> <span class=tag>${d.country}</span><div><b>${d.title}</b></div><div class=muted>${d.source} · ความเสี่ยง ${d.risk}</div><a href="${d.url}" target=_blank rel=noopener>เปิดแหล่งต้นทาง</a></div>`).join('')||'<div class=muted style="margin-top:10px">ยังไม่มีรายการ กด “ค้นหาให้ตอนนี้”</div>'}async function scan(){scanBtn.disabled=true;scanBtn.textContent='กำลังค้นหา...';await fetch('/api/search');await load();scanBtn.disabled=false;scanBtn.textContent='ค้นหาให้ตอนนี้'}async function testPaypal(){paypalBtn.disabled=true;paypalBtn.textContent='กำลังทดสอบ...';let r=await fetch('/api/paypal/test').then(r=>r.json());paypal.textContent=r.message+(r.ok?' ✅':' ❌');paypalBtn.disabled=false;paypalBtn.textContent='ทดสอบ PayPal Sandbox'}load()</script></body></html>'''
